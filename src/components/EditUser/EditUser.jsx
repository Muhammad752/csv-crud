        ...
        <div>
          ...
          <div className='addUser__data mb-12'>
            ...
            <div>
              ...
              <input id="role_user_read" type="checkbox" 
              checked={roles.ROLE_USER_READ_FILE}
              onChange={(ev)=>setRoles({...roles,"ROLE_USER_READ_FILE":ev.target.checked})
            }
              />
              </div>
              <div>
              <label htmlFor="role_user_create">CREATE</label>
              <input id="role_user_create" type="checkbox" 
              checked={roles.ROLE_USER_CREATE_FILE} 
              onChange={(ev)=>setRoles({...roles,"ROLE_USER_CREATE_FILE":ev.target.checked})}
              />
              </div>
              <div>
              <label htmlFor="role_user_update">UPDATE</label>
              <input id="role_user_update" type="checkbox" 
              checked={roles.ROLE_USER_UPDATE_FILE} 
              onChange={(ev)=>setRoles({...roles,"ROLE_USER_UPDATE_FILE":ev.target.checked})}
              />
              </div>
              <div>
              <label htmlFor="role_user_delete">DELETE</label>
              <input type="checkbox" id="role_user_delete"
              checked={roles.ROLE_USER_DELETE_FILE} 
              onChange={(ev)=>setRoles({...roles,"ROLE_USER_DELETE_FILE":ev.target.checked})}
              />
              </div>
              </div>
                <div className="flex gap-4 ml-3">
              ...
              <input type="checkbox" id="role_admin" 
            
              checked={roles.ROLE_ADMIN} 
              onChange={(ev)=>setRoles({...roles,"ROLE_ADMIN":ev.target.checked,"ROLE_ADMIN_CREATE_USER":ev.target.checked,"ROLE_ADMIN_DELETE_USER":ev.target.checked,"ROLE_ADMIN_READ_USER":ev.target.checked,"ROLE_ADMIN_UPDATE_USER":ev.target.checked})}
              />
              </div>
              <div>
              <label htmlFor="role_admin_read">READ</label>
              <input id="role_admin_read" type="checkbox" 
              
              checked={roles.ROLE_ADMIN_READ_USER} 
              onChange={(ev)=>setRoles({...roles,"ROLE_ADMIN_READ_USER":ev.target.checked})}/>
              </div>
              <div>
              <label htmlFor="role_admin_create">CREATE</label>
              <input type="checkbox" id="role_admin_create"
              
              checked={roles.ROLE_ADMIN_CREATE_USER} 
              onChange={(ev)=>setRoles({...roles,"ROLE_ADMIN_CREATE_USER":ev.target.checked})}
              />
              </div>
              <div>
              ...
              onChange={(ev)=>setRoles({...roles,"ROLE_ADMIN_UPDATE_USER":ev.target.checked})}
              />
              </div>
              <div>
              <label htmlFor="role_admin_delete">DELETE</label>
              <input type="checkbox" id="role_admin_delete" 
              checked={roles.ROLE_ADMIN_DELETE_USER} 
              onChange={(ev)=>setRoles({...roles,"ROLE_ADMIN_DELETE_USER":ev.target.checked})}
              />
              </div>
              </div>
</div>
